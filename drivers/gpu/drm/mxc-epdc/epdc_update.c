...
static irqreturn_t mxc_epdc_irq_handler(int irq, void *dev_id)
{
	...
	dev_dbg(priv->drm.dev, "EPDC interrupts fired = 0x%x, "
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	update_addr = priv->epdc_mem_phys +
		((upd_region->top * priv->epdc_mem_width) +
		upd_region->left);
				   ...
				   struct mxc_epdc *priv)
{
	...
	mutex_lock(&priv->queue_mutex);
	if ((upd_data->update_region.left + upd_data->update_region.width > priv->epdc_mem_width) ||
		(upd_data->update_region.top + upd_data->update_region.height > priv->epdc_mem_height)) {
		mutex_unlock(&priv->queue_mutex);
		dev_err(priv->drm.dev,
	...
	if (priv->waiting_for_idle) {
		dev_dbg(priv->drm.dev, "EPDC not active."
	...
	if (upd_data->update_marker != 0) {
		...
		marker_data = kzalloc(sizeof(struct update_marker_data),
				GFP_KERNEL);
		if (!marker_data) {
			dev_err(priv->drm.dev, "No memory for marker!\n");
			mutex_unlock(&priv->queue_mutex);
			return -ENOMEM;
		}
...
static void epdc_intr_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < priv->num_luts; i++) {
		...
		if (!wb_lut_done)
			list_for_each_entry_safe(next_marker, temp,
				...
				dev_dbg(priv->drm.dev, "Signaling marker %d\n",
	...
	if (epdc_waiting_on_wb) {
		...
		if (priv->cur_update->update_desc->upd_data.flags
			& EPDC_FLAG_TEST_COLLISION) {
			...
			list_for_each_entry_safe(next_marker, temp,
				...
				/* Signal completion of update */
				dev_dbg(priv->drm.dev, "Signaling marker "
					"for dry-run - %d\n",
					next_marker->update_marker);
				complete(&next_marker->update_completion);
			}
			...
			list_for_each_entry_safe(next_marker, temp,
				&priv->cur_update->update_desc->upd_marker_list,
				upd_list) {
