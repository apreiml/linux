...
void mxc_epdc_powerup(struct mxc_epdc *priv)
{
	...
	if (IS_ERR((void *)ret)) {
		dev_err(priv->drm.dev, "Unable to enable V3P3 regulator."
			"err = 0x%x\n", ret);
		mutex_unlock(&priv->power_mutex);
		return;
	}
	...
	if (IS_ERR((void *)ret)) {
		dev_err(priv->drm.dev, "Unable to enable DISPLAY regulator."
			"err = 0x%x\n", ret);
		mutex_unlock(&priv->power_mutex);
		return;
	}
	...
	if (IS_ERR((void *)ret)) {
		dev_err(priv->drm.dev, "Unable to enable VCOM regulator."
			"err = 0x%x\n", ret);
		mutex_unlock(&priv->power_mutex);
		return;
	}
...
{
#warning this should come from panel
	struct imx_epdc_fb_mode epdc_m = {
	4,            /* vscan_holdoff */
	10,          /* sdoed_width */
	20,          /* sdoed_delay */
	10,          /* sdoez_width */
	...
	struct imx_epdc_fb_mode *epdc_mode = &epdc_m;
	u32 reg_val;
	int num_ce;
	int i;
	...
	if (priv->rev >= 30) {
		...
		epdc_write(priv, EPDC_WB_FIELD2, /* 110 00101 0101 0100*/ 0xc443);
		epdc_write(priv, EPDC_WB_FIELD1, /* 101 00000 0000 0100*/ 0xa003);
		}
	}
