...
static ssize_t twl4030_usb_vbus_out_store(struct device *dev, struct device_attribute *attr,
                const char *buf, size_t n)
{
	...
	extern void twl4030_charger_enable_usb_ext(bool);
		...
		twl4030_charger_enable_usb_ext(0);
        	...
        	twl4030_i2c_access(twl, 1);
                twl4030_usb_clear_bits(twl,ULPI_OTG_CTRL,ULPI_OTG_CTRL_DRVVBUS);
/*
		twl4030_usb_set_bits(twl, ULPI_OTG_CTRL, ULPI_OTG_ID_PULLUP);
		twl4030_usb_clear_bits(twl,ULPI_USB_INT_EN_FALL,0xe);
		twl4030_usb_clear_bits(twl,ULPI_USB_INT_EN_RISE,0x8);
		twl4030_usb_set_bits(twl,ULPI_USB_INT_EN_FALL,0x10);
*/
        	twl4030_i2c_access(twl, 0);
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
		...
		return -EINVAL;
       	...
       	twl4030_i2c_access(twl, 1);
	if (fake_host)
		twl4030_usb_set_bits(twl,ULPI_OTG_CTRL,ULPI_OTG_DP_PULLDOWN_DIS);
	else
		twl4030_usb_clear_bits(twl,ULPI_OTG_CTRL,ULPI_OTG_DP_PULLDOWN_DIS);
       	twl4030_i2c_access(twl, 0);
	return n;
}
...
static DEVICE_ATTR(fake_host, 0644, twl4030_usb_fake_host_show, twl4030_usb_fake_host_store);
